...
Many interesting words are hidden away in `.pdf`s. The same goes with `.doc`, `.docx`, `.html`, `.epub`, `.rtf` and common file types. Each type uses special conventions to record margin size, font size, bold text and italic text. Yet, some researchers only need to access the words. 
...
```
Tika has detected the file format, parsed it appropriately, and extracted a plain text rendition. Use the `cat` function to see the characters as well as the blank lines:
```{r,comment= "#>" }
# Display a substring of the first text for this example.
cat(substr(text[1],45,160))
```

The `#>` are not part of the text and are used in this vignette to indicate R's output. The plain text to the right of the `#>` reveals textual content that retains only minimal formatting, such as empty lines.

Behind the scenes, each character in every word is encoded in UTF-8. That is a standard character encoding in R. It's the most commonly used encoding on the Internet (See: <https://en.wikipedia.org/wiki/UTF-8>).

One approach for studying words in English is to assume that words are merely groups of letters separated by other characters. By "other characters", I mean punctuation, line breaks, paragraph indentions, and non alpha-numeric characters. UTF-8 can represent 1,112,064 codes, and only a small number are in English letters. Fortunately, in base R we can implement the general idea in a line of code using an old computer language called regular expressions:
...
Nice! It was pretty easy to get some words from plain text. There is a lot of computing action in a small space. From the innermost function to the outside, the `tolower` function turns all the text to lowercase. This implies that capitalization changes in a word don't change the meaning of that word enough to account for. It is one of several simplifying assumptions.

Next, `strsplit` takes the now lowercase `text` string, and *splits* it into many individual "words". The split parameter `\\W+` is a regular expression that splits on any character that is *not* part of an English alpha-numeric word. Note that the `[[1]]`  grabs the first character vector from the `list`  produced by `strsplit`.

This would be more difficult if it were not plain text. Consider `HTML`, the most common format on the internet. `HTML` uses code like `<b>word</b> <i>word</i>` to stylize and `<div>word</div><div>word</div>` to format lines. Getting the words requires knowing something about how they are displayed (for more details on `HTML` to text rendering, see: <https://html.spec.whatwg.org/multipage/dom.html#the-innertext-idl-attribute>).

So far, we have a list of words that includes numbers. To ignore numbers, we can change the regular expression. Before, we split with `\\W+`. That means *not* alpha-numeric (it's a shortcut for  regular expression `[^A-Za-z0-9]+`), and we can change our code to mean *not* a character:
...
```
 A factor represents words and other categorical data. It creates an implicit dictionary, where each unique word is put into alphabetical order and then assigned an integer to identify it. You can see this dictionary using the `level` command on a factor. By the way, it's worth spending a few minutes reading the  `?factor` help.
...
Text is a sequence of categories. What if we create a model of the the first words in the document? I'm going to create a hypothetical case that demonstrates why this is tricky. Let's start small with the first `n` words, say `n=3`, and put them into a table for analysis.

```{r}
document_1 <- data.frame(word_1 = words[1], word_2 = words[2], word_3 = words[3])
document_1
```
Next, we turn the table into a matrix of numbers. This is implicitly done in most R models. It usually happens without having to think about it, in almost any model, including linear, machine learning, neural network and clustering.
```{r}
...
```
The famous Titan computer built in 2012 for $60 million has 693.5 tebibyte of RAM (See: <https://en.wikipedia.org/wiki/Titan_(supercomputer)>), which is still not enough. A smarter approach is needed. Only a handful of models can handle big data gracefully, such as sparse matrix methods. 
...
There are some models in R that use sparse matrices, such as `XGBoost`, `MatrixModels`, or `Glmnet`. A more general technique is to divide the data into smaller subsets of observations, and optimize bit by bit. This latter method can be done in the `keras` and `h2o` packages. 
...
This simplifies analysis dramatically. It assumes word order does not matter enough to warrant the cost, but given the cost of a supercomputer, that might be the case. Let's count how many times words are in this document:
```{r}
...
Word counts will use fewer parameters in your model. This one uses less than a kilobyte of RAM, instead of many terabytes.
...
The `data.table` package was developed to handle huge tabular data in memory, and is sometimes an order of magnitude faster than R's built in data.frame. `data.table` will come in handy for counting lots of words. Nota bene: the syntax is very succinct.

```{r}
library('data.table')
```
Let's get word counts. Recall that the `text` is a character vector of two long strings, one for each document:
```{r}
started.at <- proc.time()
# A table at the 'document' level, with one row per document. Give each document a short name.
...
# You can use any of these variables this way:
documents <- dcast(document_words
              , as.formula("document ~ word")
              , fill=0
...
Previously inaccessible text now has a lot of research potential, and I hope this fact arouses your interest. For those that are working with documents that have embedded tables, or important document structure and metadata, `tika` can retain much more information with its `json` and `xml` output modes.
