...
There is also some room for improvement in the Tika software. The R community uses Latex and Markdown, but Tika currently has limited support for parsing those formats.  Tika reads the files just fine, and outputs text that is essentially similar to the input file. However, Tika does not yet have Java parsers to fully understand the Latex or Markdown document structure, render it to XHTML, or extract just the plain text and ignore the markup.  For these cases, a tool such as Pandoc may help more. 
