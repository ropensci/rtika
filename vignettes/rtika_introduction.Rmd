...
There is also some room for improvement in the Tika software. The R community uses Latex and Markdown, but Tika currently has limited support for parsing those formats. Tika reads the actual files just fine, and outputs text that is essentially the same as the input file. It also captures metadata and recognizes the MIME type when downloading with `tika_fetch()`. However, Tika does not have Java parsers to fully understand the Latex or Markdown document structure, render it to XHTML, or extract just the plain text and ignore the markup. For these cases, Pandoc will be more useful (See: https://pandoc.org/demos.html ). 
